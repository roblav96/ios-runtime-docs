...
The Objective-C Interfaces are exposed as JavaScript classes. Each Objective-C interface is presented by a corresponding JavaScript constructor function and prototype object. The methods declared in interfaces are exposed:
 - On the constructor function if static
 - On the prototype if instance
JavaScript properties are declared on the prototype for Objective-C properties.
...
### New
The *new* keyword used with JavaScript constructor function for Objective-C class will try to locate an appropriate init method, and do an alloc().init(). However the more complex initializers can make it hard to select one at runtime. Also there is a cost for the performed lookup. We would like to discourage you from using *new* but in some simple cases you can:

``` JavaScript
// Will do an UIView.alloc().init();
var view = new UIView();

// Will do an UIView.alloc().initWithFrame()
var dictionary = new UIView(CGRectMake({ x: 10, y: 10, width: 200, height: 100}));
```
...
In Objective-C the default generated getter methods have the name of the property and the setter methods have "set<property>:". Because of that collision the getter and setter methods for properties are shadowed and are not exposed as JavaScript functions.
